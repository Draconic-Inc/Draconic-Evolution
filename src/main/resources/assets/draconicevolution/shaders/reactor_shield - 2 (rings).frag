...
//
//
//    //The value used to determine this pixels ultimate colour.
////    float value = sin(((coord.y * M_PI * 32)) + (time * 8));
//    float up = mod(sin(((coord.y * M_PI * 32)) + (time * 8)), 1);
//    float down = mod(sin(((coord.y * M_PI * 32)) + (time * -8)), 1);
//
//    float baseR = 1;
//    float baseG = 1;
//    float baseB = 1;
//    vec4 colour = vec4(baseR * down, (baseG * up) - down, (baseB * up) + (baseB * down), 1);
//    colour.a = (colour.r + colour.g + colour.b) / 3;
//	gl_FragColor = colour;

//    float uPos = (cos(time * 2) + 1) / 2;
//    float dPos = (cos(time * 2) + 1) / 2;
    float uPos = mod(time * 2, 1);
    float dPos = mod(time * -2, 1);

    float u = (1 - (abs(uPos - coord.y) * 50));
    float d = (1 - (abs(dPos - coord.y) * 50));

    vec4 colour = vec4(0, 1, 1, 1);
    colour.a = max(d, max(u, 0.2));
    gl_FragColor = colour;
}
