...
public class PortalHelper {
	...
	public static boolean isFrame(Block block) { return block == ModBlocks.infusedObsidian; }

	public static boolean isReceptacle(Block block) { return block == ModBlocks.dislocatorReceptacle; }

	public static boolean isPortal(Block block) { return block == ModBlocks.portal; }
	...
	public static PortalStructure traceFrame(World world, int x, int y, int z, ForgeDirection startDir, ForgeDirection plane){
		...
		for (int i = 0; i <= MAX_SIZE; i++){
			...
			else if (!world.isAirBlock(startX + i * startDir.offsetX, startY + i * startDir.offsetY, startZ + i * startDir.offsetZ)) return null;
		}
		...
		for (int i = 0; i <= MAX_SIZE; i++){
			...
			else if (!world.isAirBlock(startX + i * plane.offsetX, startY + i * plane.offsetY, startZ + i * plane.offsetZ)) return null;
		}
		...
		if (!structure.checkFrameIsValid(world, x, y, z) || !structure.scanPortal(world, x, y, z, false, false)) return null;
