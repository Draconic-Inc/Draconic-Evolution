	...
	public void tickStart(TickEvent.PlayerTickEvent event) {
		...
		if (!playerNotified && thread.isComplete())
		{
			...
			if (!thread.getVersion().equals(VersionHandler.VERSION) || (VersionHandler.SNAPSHOT > 0 && thread.getSnapshot() == 0))
			{
				...
				if (!StringUtils.isNullOrEmpty(thread.getNote())) event.player.addChatComponentMessage(new ChatComponentText(thread.getNote()));
			}
		...
		} else if (thread.isFailed())
		{
			...
			if (!StringUtils.isNullOrEmpty(thread.getNote())) event.player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.RED + thread.getNote()));
		}
