...
public class EntityChaosVortex extends Entity {
    ...
    public void onUpdate() {
        ...
        if (ticksExisted < 30 && ticksExisted % 5 == 0 && worldObj.isRemote)
            DraconicEvolution.proxy.spawnParticle(
                    new Particles.ChaosExpansionParticle(worldObj, posX, posY, posZ, false), 512);
        if (ticksExisted >= 100 && ticksExisted < 130 && ticksExisted % 5 == 0 && worldObj.isRemote)
            DraconicEvolution.proxy.spawnParticle(
                    new Particles.ChaosExpansionParticle(worldObj, posX, posY, posZ, true), 512);
    ...
    private void shakeScreen() {
        ...
        List<EntityPlayer> players =
                worldObj.getEntitiesWithinAABB(EntityPlayer.class, boundingBox.expand(200, 200, 200));
