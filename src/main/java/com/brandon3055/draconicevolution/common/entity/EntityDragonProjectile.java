...
public class EntityDragonProjectile extends Entity {
	...
	public static final int TELEPORT = 2;			/** Ender pearl which teleports the player away if it hits. */
	...
	protected void entityInit() {
		if (type == ENERGY_CHASER || type == CHAOS_CHASER || type == MINI_CHAOS_CHASER || type == IGNITION_CHARGE || worldObj.isRemote) noClip = true;
	...
	public void onUpdate() {
		...
		if (target == null){
			if (worldObj.getClosestPlayer(posX, posY, posZ, 60) != null) target = worldObj.getClosestPlayer(posX, posY, posZ, 60);
			else {
	...
	public boolean attackEntityFrom(DamageSource source, float dmg) {
		...
		if ((source.getEntity() instanceof EntityPlayer || source.getEntity() instanceof EntityArrow) && ticksExisted > 5) heath -= dmg;
	...
	public int getParticleColour(){
		switch (type){
			case FIREBOMB: return 0xFF6600;
			case TELEPORT: return 0;
			case FIRE_CHASER: return 0xFF6600;
			case ENERGY_CHASER: return 0x00FFFF;
			case CHAOS_CHASER: return 0x440000;
			case MINI_CHAOS_CHASER: return 0x440000;
			case IGNITION_CHARGE: return 0xFFFFFF;
		}
		return 0;
	}
