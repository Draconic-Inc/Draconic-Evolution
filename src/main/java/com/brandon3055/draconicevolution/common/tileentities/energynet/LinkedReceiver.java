...
public class LinkedReceiver {
	...
	public int receiveEnergy(World world, int maxReceive, boolean simulate) {
		if (isStillValid(world)) return getReceiver(world).receiveEnergy(ForgeDirection.getOrientation(connectionSide), maxReceive, simulate);
		else return 0;
	}

	public int getEnergyStored(World world) {
		if (isStillValid(world)) return getReceiver(world).getEnergyStored(ForgeDirection.getOrientation(connectionSide));
		else return 0;
	}

	public int getMaxEnergyStored(World world) {
		if (isStillValid(world)) return getReceiver(world).getMaxEnergyStored(ForgeDirection.getOrientation(connectionSide));
		else return 0;
	}
