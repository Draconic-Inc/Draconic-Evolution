...
public abstract class TileGate extends TileObjectSync implements IDEPeripheral {
	...
	public Object[] callMethod(String method, Object... args) {
		if (method.equals("getFlow")){
			...
			if (args.length == 0 || !(args[0] instanceof Boolean)) throw new IllegalArgumentException("Expected Boolean got "+(args.length == 0 ? "nil" : args[0].getClass().getSimpleName()));
			...
			if (args.length == 0 || !(args[0] instanceof Number)) throw new IllegalArgumentException("Expected Number got "+(args.length == 0 ? "nil" : args[0].getClass().getSimpleName()));
