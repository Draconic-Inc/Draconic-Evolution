...
public class TileEnergyStorageCore extends TileObjectSync {
	...
	private boolean testForOrActivateDraconium(int x, int y, int z, boolean set, boolean activate){
		if (!activate) {
			if (set) {
				...
				return true;
			} else return worldObj.getBlock(x, y, z) == ModBlocks.draconiumBlock || (worldObj.getBlock(x, y, z) == ModBlocks.invisibleMultiblock && worldObj.getBlockMetadata(x, y, z) == 0);
		}else{
	...
	private void detectAndRendChanges(){
		if (lastTickCapacity != energy) lastTickCapacity = (Long) sendObjectToClient(References.LONG_ID, 0, energy, new NetworkRegistry.TargetPoint(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 20));
	}
