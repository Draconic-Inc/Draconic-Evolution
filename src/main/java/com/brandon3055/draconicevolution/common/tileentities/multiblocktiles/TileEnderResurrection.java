...
import com.brandon3055.draconicevolution.client.render.Particles.AdvancedSeekerParticle;
...
public class TileEnderResurrection extends TileEntity{
	...
	private void spawn(){
		...
		for (int i = 0; i < diamondPillars.length; i++){
			...
			if (!worldObj.isRemote) worldObj.createExplosion(owner, diamondPillars[i].getXCoord() + 0.5, diamondPillars[i].getYCoord() + 0.5, diamondPillars[i].getZCoord() + 0.5, 3, true);
		}
		for (int i = 0; i < draconiumPillars.length; i++){
			...
			if (!worldObj.isRemote) worldObj.createExplosion(owner, draconiumPillars[i].getXCoord() + 0.5, draconiumPillars[i].getYCoord() + 0.5, draconiumPillars[i].getZCoord() + 0.5, 3, true);
		}
	...
	private void lEffects() {
		...
		if (worldObj.isRemote)
			p.setLocationAndAngles(p.posX, p.posY-1.6, p.posZ, yaw, pitch);
	}
				...
				if (worldObj.getBlock(x, y, z) == Blocks.bedrock){
					...
					for (int x1 = x-1; x1 <= x+1; x1++){
						for (int y1 = y-1; y1 <= y+1; y1++){
							for (int z1 = z-1; z1 <= z+1; z1++){
								if ((x1 != x || y1 != y || z1 != z) && worldObj.getBlock(x1, y1, z1) == Blocks.bedrock) flag = false;
							}
						}
					}
					...
					if (flag && !worldObj.isRemote) {
						if (spawnCrystal) {
							...
							worldObj.addWeatherEffect(new EntityLightningBolt(worldObj, x, y + 1, z));
					}
				}
			}
		}
	...
	private boolean isBaseValid(){
		if (worldObj.getBlock(xCoord+1, yCoord, zCoord) != Blocks.obsidian || worldObj.getBlock(xCoord-1, yCoord, zCoord) != Blocks.obsidian || worldObj.getBlock(xCoord, yCoord, zCoord+1) != Blocks.obsidian || worldObj.getBlock(xCoord, yCoord, zCoord-1) != Blocks.obsidian) return false;
		if (worldObj.getBlock(xCoord+1, yCoord, zCoord+1) != Blocks.glowstone || worldObj.getBlock(xCoord-1, yCoord, zCoord-1) != Blocks.glowstone || worldObj.getBlock(xCoord-1, yCoord, zCoord+1) != Blocks.glowstone || worldObj.getBlock(xCoord+1, yCoord, zCoord-1) != Blocks.glowstone) return false;
		return true;
	}
	...
	private boolean arePillarsValid(){
		for (int i = 0; i < diamondPillars.length; i++){
			...
			if (worldObj.getBlock(diamondPillars[i].getXCoord(), diamondPillars[i].getYCoord(), diamondPillars[i].getZCoord()) != Blocks.diamond_block || worldObj.getBlock(diamondPillars[i].getXCoord(), diamondPillars[i].getYCoord()-1, diamondPillars[i].getZCoord()) != Blocks.quartz_block) return false;
		}
		for (int i = 0; i < draconiumPillars.length; i++){
	...
	private boolean isPillarValid(int type, int x, int y, int z){
		if (type == 0)
		{
			if (worldObj.getBlock(x, y-1, z) == Blocks.quartz_block && worldObj.getBlock(x, y-2, z) == Blocks.quartz_block && worldObj.getBlockMetadata(x, y-1, z) == 2 && worldObj.getBlockMetadata(x, y-2, z) == 2) return true;
		}else if (type == 1)
		{
			if (worldObj.getBlock(x, y-1, z) == Blocks.quartz_block && worldObj.getBlockMetadata(x, y-1, z) == 2) return true;
		}
		return false;
	}
	...
	public void writeToNBT(NBTTagCompound compound) {
		...
		for (int i = 0; i < diamondPillars.length; i++){
			if (diamondPillars[i] != null)
				diamondPillars[i].writeToNBT(compound, String.valueOf(i));
		}
		for (int i = 0; i < draconiumPillars.length; i++){
			if (draconiumPillars[i] != null)
				draconiumPillars[i].writeToNBT(compound, "D"+String.valueOf(i));
		}
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		for (int i = 0; i < diamondPillars.length; i++){
			if (diamondPillars[i] != null)
				diamondPillars[i].readFromNBT(compound, String.valueOf(i));
		}
		for (int i = 0; i < draconiumPillars.length; i++){
			if (draconiumPillars[i] != null)
				draconiumPillars[i].readFromNBT(compound, "D"+String.valueOf(i));
		}
	}
