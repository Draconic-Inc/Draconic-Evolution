...
public class TileDislocatorReceptacle extends TileEntity implements IInventory {
	...
	public void updateEntity() {
		...
		if (ticksTillStart == 0) worldObj.scheduleBlockUpdate(xCoord, yCoord, zCoord, worldObj.getBlock(xCoord, yCoord, zCoord), 1);
	}
	...
	public void updateState(){
		if (structure == null || !isActive) structure = PortalHelper.getValidStructure(worldObj, xCoord, yCoord, zCoord);
		if (structure == null) {
			isActive = false;
		...
		if (isActive){
			if (getStackInSlot(0) == null || !structure.checkFrameIsValid(worldObj, xCoord, yCoord, zCoord)) isActive = false;
		}else {
	...
	public Teleporter.TeleportLocation getLocation(){
		if (getStackInSlot(0) != null && getStackInSlot(0).getItem() instanceof TeleporterMKI) return ((TeleporterMKI)getStackInSlot(0).getItem()).getLocation(getStackInSlot(0));
		return null;
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("Dislocator")) dislocator = ItemStack.loadItemStackFromNBT(compound.getCompoundTag("Dislocator"));
