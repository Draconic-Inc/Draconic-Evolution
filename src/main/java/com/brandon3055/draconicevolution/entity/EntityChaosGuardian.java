...
import static net.minecraft.util.EntityPredicates.CAN_AI_TARGET;
...
public class EntityChaosGuardian extends EntityDragonOld {
    ...
    private final ServerBossInfo bossInfo = (ServerBossInfo) new ServerBossInfo(this.getDisplayName(), BossInfo.Color.RED, BossInfo.Overlay.PROGRESS).setDarkenSky(true).setCreateFog(true).setPlayEndBossMusic(true);

    public EntityChaosGuardian(EntityType<EntityChaosGuardian> type, World world) {
        super(type, world);
        enablePersistence();
    }
...
//    }
    ...
    protected void registerData() {
        super.registerData();
        this.dataManager.register(CRYSTAL_POSITION, Optional.empty());
    }
...
//            this.world.spawnParticle("largeexplode", this.getPosX() + (double) f, this.getPosY() + 2.0D + (double) f1, this.getPosZ() + (double) f2, 0.0D, 0.0D, 0.0D);
        ...
        f = 0.2F / (MathHelper.sqrt(this.getMotion().getX() * this.getMotion().getX() + this.getMotion().getZ() * this.getMotion().getZ()) * 10.0F + 1.0F);
        f *= moveSpeedMultiplier == 0 ? 1 : moveSpeedMultiplier;
        f *= (float) Math.pow(2.0D, this.getMotion().getY());
        ...
        if (this.world.isRemote) {
            ...
            if (this.target != null) {
                ...
                this.targetY = this.target.getBoundingBox().minY + d8 + (behaviour == EnumBehaviour.CIRCLE_PLAYER ? 25 : 0);
            }
            ...
            this.setMotion(this.getMotion().add(0.0D, d0 * 0.10000000149011612D, 0.0D));
            this.rotationYaw = MathHelper.wrapDegrees(this.rotationYaw);
            ...
            float f6 = MathHelper.sqrt(this.getMotion().x * this.getMotion().x + this.getMotion().z * this.getMotion().z) * 1.0F + 1.0F;
            double d9 = Math.sqrt(this.motionMultiplier.x * this.motionMultiplier.x + this.motionMultiplier.z * this.motionMultiplier.z) * 1.0D + 1.0D;
...
//        this.dragonPartWing2.width = 4.0F;
        ...
        if (!this.world.isRemote && this.hurtTime == 0) {
            ...
            this.attackEntitiesInList(this.world.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartHead.getBoundingBox().grow(1.0D, 1.0D, 1.0D)));
        }
...
//ignitionChargeTimer = 10;
        ...
        if (ignitionChargeTimer > 1 || (ignitionChargeTimer == 1 && ticksExisted % 20 == 0) && !DEOldConfig.disableGuardianCrystalRespawn) {
            ignitionChargeTimer--;
        }
    ...
    private void updateAttack() {
        ...
        if (behaviour == EnumBehaviour.FIREBOMB && Utils.getDistanceAtoB(getPosX(), getPosY(), getPosZ(), homeX, homeY + 30, homeZ) <= 3) {
            ...
            if (targets != null && targets.size() > 0) {
                targets.removeIf(player -> player.abilities.isCreativeMode);
            }
    ...
    private void setNewTarget() {
        ...
        switch (behaviour) {
            ...
            case LOW_HEALTH_STRATEGY:
                ...
                break;
            case FIREBOMB:
                @SuppressWarnings("unchecked") List<PlayerEntity> targets = world.getEntitiesWithinAABB(PlayerEntity.class, getBoundingBox().grow(150, 150, 150), CAN_AI_TARGET);
                target = null;
...
//				connectedCrystalID = -1;
    ...
    private boolean destroyBlocksInAABB(AxisAlignedBB par1AxisAlignedBB) {
            ...
            for (int y = j; y <= i1; ++y) {
                for (int z = k; z <= j1; ++z) {
                    ...
                    if (!world.isAirBlock(pos) && !DESTRUCTION_BLACKLIST.contains(block)) {
                        if (block.canEntityDestroy(state, world, pos, this) && this.world.getGameRules().getBoolean(GameRules.MOB_GRIEFING)) {
                            flag1 = this.world.removeBlock(pos, false) || flag1;
                        }
                        else {
                            flag = true;
                        }
        ...
        if (flag1) {
            ...
            this.world.addParticle(ParticleTypes.EXPLOSION_EMITTER, d1, d2, d0, 0.0D, 0.0D, 0.0D);
        }
...
//        LogHelper.bigDev(String.format("ChaosGuardian: Save chunkCoord:(%s, %s) actualChunkCoord:(%s, %s) x=%s, z=%s", chunkCoordX, chunkCoordZ, chunkX, chunkZ, getPosX(), getPosZ()));
    ...
    public void readAdditional(CompoundNBT compound) {
        ...
        if (compound.contains("Behaviour")) behaviour = EnumBehaviour.valueOf(compound.getString("Behaviour"));
...
//        LogHelper.bigDev(String.format("ChaosGuardian: Load chunkCoord:(%s, %s) actualChunkCoord:(%s, %s) x=%s, z=%s", chunkCoordX, chunkCoordZ, chunkX, chunkZ, getPosX(), getPosZ()));
    ...
    public void setCrystalPos(@Nullable BlockPos pos) {
        this.getDataManager().set(CRYSTAL_POSITION, Optional.ofNullable(pos));
    }

    @Nullable
    public BlockPos getCrystalPos() {
        return (BlockPos) ((Optional) this.getDataManager().get(CRYSTAL_POSITION)).orElse(null);
    }
