...
public class TilePortalClient extends TilePortal implements ITickableTileEntity {
    ...
    public void tick() {
        ...
        double distanceMod = Utils.getDistanceAtoB(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, player.getPosX(), player.getPosY(), player.getPosZ());
...
//                BCEffectHandler.spawnFX(DEParticles.PORTAL, world, new Vec3D(pos).add(1, rD1, rD2), new Vec3D(pos).add(0.25, rD1 + rO1, rD2 + rO2), 256D);
            ...
            else if (axis == Y && player.getPosY() + player.getEyeHeight() > pos.getY() + 0.5) {
