 ...
 */
public class TileCraftingCore extends TileBCore implements IFusionInventory, IFusionStateMachine, ITickableTileEntity, INamedContainerProvider, IActivatableTile, IChangeListener {
    ...
    private final ManagedInt fusionCounter = register(new ManagedInt("fusion_counter", SAVE_NBT_SYNC_TILE));
    ...
    private List<IFusionInjector> injectors = new ArrayList<>();
    ...
    public TileCraftingCore() {
        ...
        itemHandler.setContentsChangeListener(i -> inventoryChanged());
    }
...
//                DraconicEvolution.LOGGER.info("State: " + getFusionState() + ", Counter: " + getCounter() + ", Charge: " + getInjectors().stream().mapToLong(IFusionInjector::getInjectorEnergy).sum());
            } else {
                cancelCraft();
            }
        }
    ...
    public void updateInjectors() {
        ...
        int radius = DEConfig.fusionInjectorRadius;
        Streams.concat(
                ...
                .map(e -> level.getBlockEntity(e))
                .filter(e -> e instanceof TileCraftingInjector)
                .forEach(tile -> {
                    ...
                    if (dist > DEConfig.fusionInjectorMinDist && Direction.getNearest((int) dirVec.x, (int) dirVec.y, (int) dirVec.z) == ((TileCraftingInjector) tile).getDirection().getOpposite() && ((TileCraftingInjector) tile).setCore(this)) {
                        BlockPos pos = tile.getBlockPos();
                        Direction facing = ((TileCraftingInjector) tile).getDirection();
                        ...
                        for (BlockPos bp : checkList) {
                            if (!level.isEmptyBlock(bp) && (level.getBlockState(bp).canOcclude() || level.getBlockEntity(bp) instanceof TileCraftingInjector)) {
                                obstructed = true;
                                ((TileCraftingInjector) tile).setCore(null);
                                break;
                            }
                        }
                        if (!obstructed) {
                            injectors.add((TileCraftingInjector) tile);
                        }
                    }
                });
    }
    ...
    public void cancelCraft() {
        crafting.set(false);
        injectors.forEach(e -> e.setEnergyRequirement(0, 0));
    }
    ...
    @Override
    public void setStateProgress(double progress, ITextComponent stateText) {
        //Todo store this data for display in GUI
    }
