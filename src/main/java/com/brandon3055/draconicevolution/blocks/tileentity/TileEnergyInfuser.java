...
public class TileEnergyInfuser extends TileEnergyInventoryBase implements IEnergyReceiver, ITickable {
    ...
    public TileEnergyInfuser(){
        ...
        registerSyncableObject(charging, false);
        registerSyncableObject(running, false);
    }
    ...
    @SideOnly(Side.CLIENT)
       private void spawnParticles(){
        if (worldObj.isRemote && running.value && charging.value) {
            for (int i = 0; i < 4; i++) {
                ...
                BCEffectHandler.spawnFX(DEParticles.INFUSER, worldObj, spawn, target, 0);

                double xRand = 0.4 + (worldObj.rand.nextDouble() * 0.7);
                double yRand = worldObj.rand.nextDouble() * 0.45;

                spawn = new Vec3D(pos).add(0.5, 0.45, 0.5).add(Math.sin(rotation) * xRand, yRand, Math.cos(rotation) * xRand);
                BCEffectHandler.spawnFX(DEParticles.INFUSER, worldObj, spawn, target, 1);
            }
        }
    }
