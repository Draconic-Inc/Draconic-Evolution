 ...
 */
public class GuiCelestialManipulator extends ContainerGuiProvider<CelestialManipulatorMenu> {
	...
	public void buildGui(ModularGui gui, ContainerScreenAccess<CelestialManipulatorMenu> screenAccess) {
		...
		Constraints.placeOutside(rsBackground, root, Constraints.LayoutPos.BOTTOM_RIGHT, 0, -dynamic(rsBackground.get(HEIGHT)).get());
		Constraints.bind(rsItem, rsBackground);
		Constraints.bind(rsButton, rsBackground);
		toolkit.addHoverHighlight(rsButton, Borders.create(0), true);
		...
		for (int ii = 0; ii < rsButtonNames.length; ii++) {
			...
			toolkit.addHoverHighlight(rsControlButtons[num], Borders.create(0), true);
	...
	public static class Screen extends ModularGuiContainer<CelestialManipulatorMenu> {
		public Screen(CelestialManipulatorMenu menu, Inventory inv, Component title) {
			super(menu, inv, new GuiCelestialManipulator());
