...
public class GuiGrinderd extends GuiContainer {

	public EntityPlayer player;
	private TileGrinder tile;

	public GuiGrinderd(InventoryPlayer invPlayer, TileGrinder tile) {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int X, int Y) {
		...
		drawCenteredString(fontRendererObj, I18n.translateToLocal(DEFeatures.grinder.getUnlocalizedName()+".name"), guiLeft + xSize/2 + 2, guiTop, 0x00FFFF);

		int x = X - guiLeft;
		int y = Y - guiTop;
		if (GuiHelper.isInRect(68, 10, 12, 40, x, y)) {
			ArrayList<String> internal = new ArrayList<String>();
			internal.add(I18n.translateToLocal("info.de.energyBuffer.txt"));
