...
public class RenderLootCore extends EntityRenderer<EntityLootCore> {
    ...
    public RenderLootCore(EntityRendererProvider.Context context) {
        super(context);
    }
...
//        ccrs.startDrawing(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX_NORMAL);
        float yOffset = Mth.sin(((float) ClientEventHandler.elapsedTicks + partialTicks) / 10.0F) * 0.1F + 0.1F;
...
//    public void renderLabel(EntityLootCore lootCore, Font renderer, float x, float y, float z, float viewY, float viewX, boolean thirdPerson) {
...
//            renderer.draw(new PoseStack(), name, 11 - (w / 2F), (int) yPos - 10, -1);
//
//            int row = 0;
...
//                RenderSystem.popMatrix();
//                renderer.draw(new PoseStack(), "x" + lootCore.displayMap.get(stack), 0, -4 + rowY, -1);
//                row++;
//            }
//        }
//
