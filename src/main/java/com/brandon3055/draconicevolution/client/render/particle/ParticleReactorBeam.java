...
public class ParticleReactorBeam extends EntityFX {
	...
	private void renderStabilizerEffect(Tessellator tessellator, float offsetX, float offsetY, float offsetZ, float length, float partialTick){
		GL11.glShadeModel(GL11.GL_SMOOTH);
		...
		drawBeam(tessellator, 1F, 0.355F, 0.8F, offsetX, offsetY, offsetZ, particleAge, partialTick, true, false, 0x00ffff);
		GL11.glPopMatrix();
		...
		float coreSize = 0.9F;
		float s = 0.355F;
		drawBeam(tessellator, s / coreSize, coreSize, length - 1F, offsetX, offsetY, offsetZ, particleAge, partialTick, false, false, 0x00ffff);
		GL11.glPopMatrix();

		GL11.glPushMatrix();
		GL11.glTranslated(0, 0, -0.35);
		drawBeam(tessellator, 1F, 0.263F, 0.8F, offsetX, offsetY, offsetZ, particleAge, partialTick, true, true, 0xff6600);
		GL11.glPopMatrix();
		...
		coreSize = 0.5F;
		s = 0.263F;
		drawBeam(tessellator, s / coreSize, coreSize, length - 0.5F, offsetX, offsetY, offsetZ, particleAge, partialTick, false, true, 0xff6600);
		GL11.glPopMatrix();
	...
	private void drawBeam(Tessellator tessellator, float sizeOrigin, float sizeTarget, float length, float offsetX, float offsetY, float offsetZ, int tick, float partialTick, boolean reverseTransparency, boolean reverseDirection, int color) {
		float speed = 3F;
		...
		for (int i = 0; i <= b0; ++i)
		{
			...
			float texU = (float)(i % b0) * 1.0F / (float)b0;
			tessellator.setColorRGBA((color & 0xFF0000) >> 16, (color & 0xFF00) >> 8, (color & 0xFF), reverseTransparency ? 0 : 255);
			tessellator.addVertexWithUV((double)(verX * sizeOrigin), (double)(verY * sizeOrigin), 0.0D, (double)texU, (double)texV1);
			tessellator.setColorRGBA((color & 0xFF0000) >> 16, (color & 0xFF00) >> 8, (color & 0xFF), reverseTransparency ? 255 : 0);
			tessellator.addVertexWithUV((double)verX, (double)verY, (double)length, (double)texU, (double)texV2);
		}
