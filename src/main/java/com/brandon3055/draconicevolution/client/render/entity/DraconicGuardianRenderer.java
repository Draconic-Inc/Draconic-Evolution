...
public class DraconicGuardianRenderer extends EntityRenderer<DraconicGuardianEntity> {
    ...
    public void render(DraconicGuardianEntity guardian, float entityYaw, float partialTicks, MatrixStack mStack, IRenderTypeBuffer getter, int packedLight) {
        ...
        if (guardian.deathTicks > 0) {
            ...
            IVertexBuilder builder = getter.getBuffer(RenderType.dragonExplosionAlpha(DRAGON_EXPLODING_TEXTURES, progress));
        ...
        if (guardian.closestGuardianCrystal != null) {
            mStack.pushPose();
            float relX = (float) (guardian.closestGuardianCrystal.getX() - MathHelper.lerp(partialTicks, guardian.xo, guardian.getX()));
            float relY = (float) (guardian.closestGuardianCrystal.getY() - MathHelper.lerp(partialTicks, guardian.yo, guardian.getY()));
            float relZ = (float) (guardian.closestGuardianCrystal.getZ() - MathHelper.lerp(partialTicks, guardian.zo, guardian.getZ()));
...
//            int shell = j / 8;
    ...
    public static class EnderDragonModel extends EntityModel<DraconicGuardianEntity> {
        ...
        public void renderToBuffer(MatrixStack mStack, IVertexBuilder getter, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
            mStack.pushPose();
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                this.spine.yRot = MathHelper.rotWrap(adouble1[0] - adouble[0]) * ((float) Math.PI / 180F) * 1.5F;
                this.spine.xRot = f9 + this.dragonInstance.getHeadPartYOffset(i, adouble, adouble1) * ((float) Math.PI / 180F) * 1.5F * 5.0F;
                this.spine.zRot = -MathHelper.rotWrap(adouble1[0] - (double) f7) * ((float) Math.PI / 180F) * 1.5F;
                this.spine.y = f3;
                this.spine.z = f4;
                this.spine.x = f2;
            ...
            this.head.xRot = MathHelper.rotWrap(this.dragonInstance.getHeadPartYOffset(6, adouble, adouble2)) * ((float) Math.PI / 180F) * 1.5F * 5.0F;
            ...
            for (int j = 0; j < 12; ++j) {
                ...
                this.spine.yRot = (MathHelper.rotWrap(adouble2[0] - adouble[0]) * 1.5F + 180.0F) * ((float) Math.PI / 180F);
                this.spine.xRot = f11 + (float) (adouble2[1] - adouble[1]) * ((float) Math.PI / 180F) * 1.5F * 5.0F;
                this.spine.zRot = MathHelper.rotWrap(adouble2[0] - (double) f7) * ((float) Math.PI / 180F) * 1.5F;
                this.spine.y = f3;
                this.spine.z = f4;
                this.spine.x = f2;
                f3 = (float) ((double) f3 + Math.sin(this.spine.xRot) * 10.0D);
