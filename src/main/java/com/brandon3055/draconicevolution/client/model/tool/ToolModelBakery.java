...
public class ToolModelBakery {
    ...
    public static IBakedModel get3DModel(ItemStack stack) {
        ...
        return modelCache.computeIfAbsent(cacheKey, k -> {
            IBakedModel model;
            try {
                model = OBJLoader.INSTANCE.loadModel(provider.getModels(stack).getValue()).bake(TRSRTransformation.identity(), DefaultVertexFormats.ITEM, TextureUtils.bakedTextureGetter);
            } catch (Exception e) {
